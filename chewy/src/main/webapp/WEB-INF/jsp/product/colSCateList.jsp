...
<!-- 			  		</div> -->
			  		...
			  		<c:forEach items="${list }" var="cate">
				  		<div class="categoryEntry">
				  			<a class="categoryEntry_label" href="selectSColCateList.do?sno=${cate.sct_no}">${cate.sct_name }</a>
...
<!-- 					</li> -->
					...
					<c:forEach items="${mList }" var="mList">
						<c:if test="${mList.mct_no eq mno }">
							<li class="active breadcrumb-item">
								<a href="selectMColCateList.do?bno=${mList.bct_no}&mno=${mList.mct_no }">${mList.mct_name } </a>
							</li>
						</c:if>
					</c:forEach>
					<c:forEach items="${sList }" var="sList">
						<c:if test="${sList.sct_no eq sno }">
							<li class="active breadcrumb-item"> ${sList.sct_name } </li>
						</c:if>
					</c:forEach>
...
<!-- 				</div> -->
				...
				<div class="ProductListHeader_headingDiv">
					<c:forEach items="${sList }" var="sList">
						<c:if test="${sList.sct_no eq sno }">
							<h1 class="ProductListHeader_headingText">${sList.bct_name} ${sList.mct_name } ${sList.sct_name }</h1>
						</c:if>
					</c:forEach>
				</div>
